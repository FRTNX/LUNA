...
def _close(connection, cursor, db = config['defaults']['db']):
    if db == 'postgresql':
        connection.close()
        cursor.close()
...
def list_titles_by_tag(tag):
    ...
    rows = cur.fetchall()
    _close(conn, cur)
    return { 'message': list(map((lambda row: row[0]), rows)) }
...
def fetch_reading_list(database = config['defaults']['db']):
    ...
    if extraction_quota > 0:
        ...
        return { 'message': pinned_articles + random_articles }
    else:
        _close(conn, cur)
        return { 'message': pinned_articles }
...
def merge_all_relations_by_origin():
    ...
    try:
        # support array of protected relations
        cur.execute("UPDATE intelligence SET origin = 'intelligence' WHERE origin NOT IN ('science')")
        _close(conn, cur)
        return 'Successfuly merged relations'
    except Exception as e:
        _close(conn, cur)
...
def fetch_document(collection, title, summary=False):
    conn, cur = _open()

    cur.execute(f'SELECT payload, flags FROM {collection} WHERE SIMILARITY (title, LOWER(%s)) > 0.7', (title.lower(),))
    row = cur.fetchone()
    ...
    #     cur.execute(f"UPDATE {collection} SET flags = array_remove(flags, 'PIN_TO_START') WHERE LOWER(title) = '{title.lower()}'")
        
    _close(conn, cur)
    
    document = row[0][0]
    if summary:
        summary_end = document.find('\n\n\n')
        return document[:summary_end]
...
def search_wikipedia_db(search_term):
    conn, cur = _open()
...
def fetch_coords():
    ...
    row = cur.fetchone()
    print('Got row:', row)
    coords = json.loads(row[0]) if row else [0,0]
    _close(conn, cur) if config['defaults']['db'] == 'postgresql' else None
    return coords
